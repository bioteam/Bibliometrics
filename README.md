...
`scrape.py` requires the firefox selenium driver on `$PATH` and `cfde_programs_key.json` in the root directory. It will generate the base data files in a `data` directory by scraping https://commonfund.nih.gov/publications?pid=83
The JSON files generated by scrape are lists of nodes of the form:
```
  {
    "program": "4D Nucleome",
    "publication_title": "Xist spatially amplifies SHARP/SPEN recruitment to balance chromosome-wide silencing and specificity to the X chromosome.",
    "published_date": "2022-03-01",
    "pmid": "35301492",
    "journal": "Nature structural & molecular biology",
    "openalex": "https://openalex.org/W4221137519",
    "total_citations": 0,
...
`affiliation.py` parses the data files from `scrape.py` and adds author affiliation data. An entry from a program JSON might look like so:
...
```
  {
    ...
    "is_open_access": true,
    "citation_counts_by_year": [],
    "last_author_openalex": "https://api.openalex.org/A2154759263",
    "last_author_raw_affil_string": "Division of Biology and Biological Engineering, California Institute of Technology, Pasadena, CA, USA. mguttman@caltech.edu.",
    "institute_name": "California Institute of Technology",
    "institute_url": "https://api.openalex.org/I122411786",
    "geo_country": "US",
...
After `affiliation.py` has added the new fields:
- last_author_openalex
- last_author_raw_affil_string
- institute_name
- geo_country
- geo_region
- author_error
